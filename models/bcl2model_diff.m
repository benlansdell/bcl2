function [conc, h1, h2, h3, h4, h5] = bcl2model_diff (k, ic, D, names, t, T_p, activator)
	% bcl2model			Solve ODE of specified Bcl2 interaction model with diffusion-limited solution-membrane
	%					interactions. Use load_model() to generate model parameters. Includes all interactions'
	%					possibly modelled -- direct activation,	Bak multimerisation. These can be 'switched off'
	%					by the appropriate parameter set requested from load_model().
	%
	% Usage:
	%					[conc, h1, h2, h3, h4, h5] = bcl2model_diff(k, ic, D, names, t, T_p, activator)
	%
	% Input:
	...
	%					D = diffusion parameter. Provided by load_model()
	...
	%
	% Examples:
	%					[k, ic, names, D] = load_model('tBid', 'reversible_exp4_diff');
	%					T_p = T_p_exp4('tBid');
	%					[conc, h1, h2, h3, h4, h5] = bcl2model_diff(k, ic, D, names, 0:5400, T_p, 'tBid');
	%					saveplot(h1, './images/test-diff.eps');
	...
	%Reactions
	%Activation of Bak
	v1 = @(x) k_f(k(1,1), k(1,2), D, x(1))*x(1)*x(2) - k(1,2)*x(6);
	%Formation of Bak:Mcl-1 complex
	v2 = @(x) k_f(k(2,1), k(2,2), D, x(6))*x(3)*x(6) - k_r(k(2,1), k(2,2), D, x(6))*x(8);
	...
	v9 = @(x) k_f(k(9,1), k(9,2), D, x(11))*x(11)*x(2) - k_r(k(9,1), k(9,2), D, x(11))*x(12);
