function T_p = T_p_exp4(activator)
	% T_p_exp4		Return tBid/tBIM spike-in derivative function for modelling Experiment 4. Every 10
	...
	T_p_ = @(t) (0.7*(t>600-window).*(t<=600) + 2*(t>1200-window).*(t<=1200) + 7*(t>1800-window).*(t<=1800) + 20*(t>2400-window).*(t<=2400) + 70*(t>3000-window).*(t<=3000))/window;
	%Additional tBid added to the experiment at 60 minutes...
	if strcmp(activator,'tBim')
		T_p = @(t) T_p_(t);
	elseif strcmp(activator,'tBid')
		T_p = @(t) T_p_(t) + 200*(t>3600-window).*(t<=3600)/window;
	else
