function [G_d, G_a] = activation_pathway_exp6(activator, model)
	...
	%
	% Usage:
	%				[G_d, G_a] = activation_pathway_exp6(activator, model)
	%
	% Input:
	...
	%
	% Examples:
	%				[G_d, G_a] = activation_pathway_exp6('tBid', 'reversible_exp6_both');

	[k, ic, names] = load_model6(activator, model);
	T_p = T_p_exp6(activator);
	conc = bcl2model(k, ic, names, (-50*60):(180*60), T_p, activator);
	tbim = conc(2,:);
	bak = conc(1,:);
	abak = conc(6,:);
