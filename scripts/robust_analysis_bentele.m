function R = robust_analysis_bentele(model, activator, filename)
	...
	% Usage:
	%
	%						R = robust_analysis_bentele(model, activator, filename)
	%
	% Input:
	...
	% 
	% Examples:
	%						R = robust_analysis_bentele('reversible_exp4_both', 'tBid', './images/test.eps');
	...
	t = (-50*60):(120*60);
	f = bcl2model(k, ic, names, t, T_p_exp4(activator), activator);
	S = zeros(13,12);
	for i=1:13
		for j=1:12
			i
			...
			f_ = bcl2model(k_, ic, names, t, T_p_exp4(activator), activator);
			c = f(z(i),:);
			c_ = f_(z(i),:);
			C = sum(c, 2);
			C_ = sum(abs(c_-c),2);
