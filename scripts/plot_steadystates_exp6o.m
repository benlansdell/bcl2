function plot_steadystates_exp6o(filename_tbim, filename_tbid)
	% plot_steadystates_exp6o	Plot equilibrium concentrations as a function of total tBim/tBid spiked
	%			as Experiment 6 proceeds. 
	%
	% Usage:
	%			plot_steadystates_exp6o(filename_tbim, filename_tbid)
	%
	% Input:
	...
	%
	% Example(s):
	%			plot_steadystates_exp6o('./images/test-tbim.eps', './images/test-tbid.eps');

	experiment = 'reversible_exp6';
	T0 = 1:300;
...
function ssdata = computeSteadyStateMatrix(model, activator, T0list)
	ssdata = [];
	[k, ic] = load_model6(activator, model);
	for T0 = T0list
		ic(2) = T0;
...
function plotsteadystate(model, activator, ssdata, filename)
	...
	if strcmp(activator,'tBid')
		F = @(t) 0.3 + 0.7*min(max(0, (t-600+window)/window), 1) + 2*min(max(0, (t-1200+window)/window), 1) + 7*min(max(0, (t-1800+window)/window), 1) + 20*min(max(0, (t-2400+window)/window), 1) + 70*min(max(0, (t-3000+window)/window), 1) + 200*min(max(0, (t-3600+window)/window), 1);
	else
		F = @(t) 3 + 7*min(max(0, (t-600+window)/window), 1) + 20*min(max(0, (t-1200+window)/window), 1) + 70*min(max(0, (t-1800+window)/window), 1) + 200*min(max(0, (t-2400+window)/window), 1);
	end
